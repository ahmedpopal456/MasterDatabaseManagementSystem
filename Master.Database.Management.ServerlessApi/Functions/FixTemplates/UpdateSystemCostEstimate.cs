...
namespace Master.Database.Management.ServerlessApi.Functions.FixTemplates
{
  public class UpdateSystemCostEstimate
  {
    ...
    public async Task<IActionResult> UpdateSystemCostEstimateAsync(Guid id, double cost, CancellationToken cancellationToken)
    {
      ...
      {
        return new BadRequestObjectResult($"{nameof(UpdateSystemCostEstimateAsync)}: The {nameof(cost)} is not a valid number...");
      }

      var result = await _requestMediatorFactory.RequestFixTemplateMediator().UpdateCostAsync(id, cost, cancellationToken);
      if (result == null)
      {
        return new NotFoundObjectResult($"{nameof(UpdateSystemCostEstimateAsync)}: Unable to update {nameof(cost)}, the requested resource with {nameof(Guid)} {id} was not found...");
      }
