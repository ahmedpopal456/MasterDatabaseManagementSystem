...
namespace Master.Database.Management.ServerlessApi.Functions.Classifications.Types
{
  public class GetTypes
  {
    ...
    {
      _requestMediatorFactory = requestMediatorFactory ?? throw new ArgumentNullException($"{nameof(GetTypes)} expects a value for {nameof(requestMediatorFactory)}... null argument was provided");

    }

    [FunctionName("GetworkTypesAsync")]
    ...
    [OpenApiResponseWithBody(HttpStatusCode.OK, "application/json", typeof(IEnumerable<WorkTypeDto>))]
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "workTypes")]
                                          ...
                                          CancellationToken cancellationToken)
    {
      ...
      return await GetWorkTypesAsync(cancellationToken, typeName, minTimestampUtc, maxTimestampUtc);
    }

    public async Task<IActionResult> GetWorkTypesAsync(CancellationToken cancellationToken, string typeName = null, string minTimestampUtc = null, string maxTimestampUtc = null)
    {
      ...
      {
        return new BadRequestObjectResult($"{nameof(GetWorkTypesAsync)}: Either {nameof(minTimestampUtc)} or {nameof(maxTimestampUtc)} is invalid...");
      }
      ...
      var result = await _requestMediatorFactory.RequestWorkTypeMediator().GetManyAsync(filterBaseDto, cancellationToken);
